...
bool llama_kv_cache::transcode_to_tg_cuda_k(const char * k_src_base, char * k_dst_base, void * stream) {
    ...
    // PP graph released: compute slack is [0, size_act_pp). KV sources live at >= size_act_pp.
...
bool llama_kv_cache::transcode_to_tg_cuda(void * stream) {
    ...
    // In-place transcoding: source is PP region, dest is TG region.
    // V regions are disjoint (TG V ends before PP V starts) — safe to forward-copy.
    // K regions overlap (TG K encompasses PP K) — transcode_to_tg_cuda_k handles
    //   this by iterating layers in reverse order.
